...
export const instructorRegister = async (req, res) => {
    const {firstName, lastName, phone, email, password} = req.body;
    try {
        const data  = await pool.query("select * from instructor where email=$1",[email]);
        ...
        let returnData = await new Promise((resolve,reject) => {
            pool.query("insert into instructor values($1, $2, $3, $4, $5, $6) returning *",queryData,(err,result) => {
                if(err)
                    reject(err);
                else{
...
export const instructorLogin = async (req,res) => {
    ...
    try {
        ...
        let data = await new Promise((resolve,reject) => {
            pool.query("select * from instructor where email=$1",queryData,(err,result) => {
                if(err)
                    reject(err);
                else{
                    resolve(result.rows[0]);
